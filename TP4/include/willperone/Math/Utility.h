 ...
 *           22-10-2012
 *
 * Special thanks to Cbloom, Graphics Gems, Braeden Shosa, Art Golf
 *
...
namespace Math
{
	...
	template <class type>
	inline type interp_lin(const type &a, const type &b, float t)
	{	return a*(1-t) + b*t; 	}

	//! cosine interpolation of 2 points
	template <class type>
	inline type interp_cos(const type &a, const type &b, float t)
	{   
		t = (1 - cos(t*PI))/2;
	    return  a*(1-t) + b*t;
	}
	...
	template <class type>
	inline type interp_cubic(const type &v0, const type &v1, const type &v2, const type &v3, float t)
	{
		type P = (v3 - v2) - (v0 - v1);
		type Q = (v0 - v1) - P;	
